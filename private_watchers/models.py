...
def user_static_wordlist_upload_path(instance, filename):
    ...
    return f"private_watchers/wordlists/{username}/static_dns_wordlist.txt"

def user_dynamic_wordlist_upload_path(instance, filename):
    username = instance.user.username if instance.user else 'anonymous'
    ext = os.path.splitext(filename)[1]
    return f"private_watchers/wordlists/{username}/dynamic_dns_wordlist.txt"
...
def user_subdomains_upload_path(instance, filename):
    ...
    ext = os.path.splitext(filename)[1]
    return f"private_watchers/subdomains/{username}/{instance.wildcard}_subdomains.txt"
...
class JSFileWatcher (BaseModel):
    watcher = models.ForeignKey(AssetWatcher , on_delete=models.CASCADE , related_name= 'jsfiles' , blank=True , null=True)
    ...
    def __str__(self):
        return f"{self.watcher.user.username} - {self.status}"
